...
// UDPMode is the UDP mode supported by the Server.
type UDPMode int

const (
	// UDPAll enables both the UDP associate command and the UDP over TCP protocol.
	UDPAll UDPMode = iota
	// UDPAssociateOnly enables only the UDP associate command.
	UDPAssociateOnly
	// UDPOverTCPOnly enables only the UDP over TCP protocol.
	UDPOverTCPOnly
	// UDPDisabled disables the UDP associate command and the UDP over TCP protocol.
	UDPDisabled
)

// Server is accepting connections and handling the details of the SOCKS5 protocol
type Server struct {
	// UDP specifies which UDP modes are accepted.
	// The zero value enables both the UDP associate command and the UDP over TCP protocol.
	UDP UDPMode
	...
	// HandshakeTimeout is the maximum amount of time to complete the SOCKS5
	// negotiation before the request is processed, protecting the server
	// from clients that connect and then stall. Zero means no timeout.
...
func (s *Server) handle(req *request) error {
	switch req.Command {
	case ConnectCommand:
		if version, ok := uotVersionOf(req.DestinationAddr); ok {
			if s.UDP == UDPAssociateOnly || s.UDP == UDPDisabled {
				if err := sendReply(req.Conn, commandNotSupported, nil); err != nil {
					return fmt.Errorf("failed to send reply: %v", err)
				}
				return errUDPOverTCPDisabled
			}
			return s.handleUoT(req, version)
		}
		return s.handleConnect(req)
	case BindCommand:
		return s.handleBind(req)
	case AssociateCommand:
		if s.UDP == UDPOverTCPOnly {
			if err := sendReply(req.Conn, commandNotSupported, nil); err != nil {
				return fmt.Errorf("failed to send reply: %v", err)
			}
			return errUDPAssociateDisabled
		}
		if s.UDP == UDPDisabled {
			if err := sendReply(req.Conn, commandNotSupported, nil); err != nil {
				return fmt.Errorf("failed to send reply: %v", err)
			}
...
// handleUoT handles a UDP over TCP request, which is a CONNECT request with
// the magic address as its destination. The UDP packets are carried by the
// connection instead of a separate UDP socket.
func (s *Server) handleUoT(req *request, version int) error {
	ctx := s.context()

	localAddr := req.Conn.LocalAddr()
	local, ok := localAddr.(*net.TCPAddr)
	if !ok {
		return fmt.Errorf("udp over tcp failed: local address is %s://%s", localAddr.Network(), localAddr.String())
	}
	bind := address{IP: local.IP, Port: local.Port}
	if err := sendReply(req.Conn, successReply, &bind); err != nil {
		return fmt.Errorf("failed to send reply: %v", err)
	...
	if version == UDPOverTCPV2 {
		var err error
		isConnect, destination, err = readUoTRequest(req.Conn)
		if err != nil {
			return err
		}
		if isConnect {
			// The destination is fixed for the whole connection, resolve it once.
			udpAddr, err := toUDPAddr(destination)
			if err != nil {
				return err
			}
			destination = &address{IP: udpAddr.IP, Port: udpAddr.Port}
		}
	}

	uotConn := NewUoTConn(req.Conn, version, isConnect, destination)
	...
	return s.relayUoT(ctx, uotConn, udpConn)
}

// relayUoT relays the UDP packets between the UDP over TCP connection and the UDP socket.
func (s *Server) relayUoT(ctx context.Context, uotConn *UoTConn, udpConn net.PacketConn) error {
	errCh := make(chan error, 2)

	go func() {
		var buf [maxUdpPacket]byte
		for {
			n, addr, err := uotConn.ReadFrom(buf[:])
			if err != nil {
				errCh <- err
				return
			}
			target, err := toUDPAddr(addr)
			if err != nil {
				if s.Logger != nil {
					s.Logger.Println(err)
				}
			...
			if _, err := udpConn.WriteTo(buf[:n], target); err != nil {
				errCh <- err
				return
			}
		}
	...
	go func() {
		...
		for {
			...
			if _, err := uotConn.WriteTo(buf[:n], addr); err != nil {
				errCh <- err
				return
			}
		}
