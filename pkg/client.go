...
type Dialer struct {
	...
	IsResolve bool
	// UDPOverTCP specifies the version of the UDP over TCP protocol used for
	// the UDP connections, it must be UDPOverTCPV1 or UDPOverTCPV2.
	// Zero disables it.
	UDPOverTCP int
...
func (d *Dialer) DialContext(ctx context.Context, network, address string) (net.Conn, error) {
	...
	case "udp", "udp4", "udp6":
		if d.UDPOverTCP != 0 {
			return d.doUDPOverTCP(ctx, address)
		}
...
// doUDPOverTCP dials the UDP over TCP protocol to reach the address,
// the UDP packets are carried by the returned connection.
func (d *Dialer) doUDPOverTCP(ctx context.Context, address string) (net.Conn, error) {
	version := d.UDPOverTCP
	if version != UDPOverTCPV1 && version != UDPOverTCPV2 {
	...
	destination, err := parseAddress(address)
	if err != nil {
		return nil, err
	}

	proxyConn, err := d.proxyDial(ctx, d.ProxyNetwork, d.ProxyAddress)
	if err != nil {
		return nil, err
	}

	conn, err := d.connect(ctx, proxyConn, ConnectCommand, net.JoinHostPort(magicAddress(version), "0"))
	if err != nil {
		proxyConn.Close()
		return nil, err
	}
	...
	if version == UDPOverTCPV2 {
		if err := writeUoTRequest(conn, true, destination); err != nil {
			conn.Close()
			return nil, err
		}
		isConnect = true
	}
	return NewUoTConn(conn, version, isConnect, destination), nil
}

// resolveAddress resolves the domain name of the address on locally.
func (d *Dialer) resolveAddress(ctx context.Context, address string) (string, error) {
	...
	return net.JoinHostPort(ipaddr[0].String(), port), nil
}
